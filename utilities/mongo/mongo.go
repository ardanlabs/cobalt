// mongo package provides mongo connectivity support.
package mongo

import (
	"github.com/OutCast-IO/Outcaster/utilities/helper"
...
func Startup(sessionId string) (err error) {
	defer helper.CatchPanicErr(&err, sessionId, "Startup")
...
func Shutdown(sessionId string) (err error) {
	defer helper.CatchPanicErr(&err, sessionId, "Shutdown")
...
func CreateSession(sessionId string, mode string, sessionName string, hosts []string, databaseName string, username string, password string) (err error) {
	defer helper.CatchPanicErr(nil, sessionId, "CreateSession")
...
func CopySession(sessionId string, useSession string) (mongoSession *mgo.Session, err error) {
	defer helper.CatchPanicErr(nil, sessionId, "CopySession")
...
func CloseSession(sessionId string, mongoSession *mgo.Session) {
	defer helper.CatchPanicErr(nil, sessionId, "CloseSession")
